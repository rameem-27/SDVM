...
def query_aadhaar(uid_value):
    try:
        with sqlite3.connect('aadhaar.db') as conn:
            cursor = conn.cursor()
            ...
            if row:
                ...
                with open(f"{uid_value}_QA.json", 'w') as result_file:
...
def compare_aadhaar_jsons(file1, file2):
