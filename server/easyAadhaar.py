...
def insert_into_aadhaar_db():
    ...
    insert_query = f"INSERT INTO details ({columns}) VALUES ({placeholders})"
    try:
...
def remove_frm_aadhaar_db():
    ...
    conn = sqlite3.connect('aadhaar.db')
    c = conn.cursor()
    c.execute("DELETE FROM details WHERE uid = ?", (uid_value,))
    conn.commit()
    conn.close()
...
def main():
    ...
    if option == '-a':
        ...
        to_json(file_data)
        insert_into_aadhaar_db()
        remove()
        
    elif option == '-r':
        file_data = read_file(filename)
        to_json(file_data)
        remove_frm_aadhaar_db()
        remove()
